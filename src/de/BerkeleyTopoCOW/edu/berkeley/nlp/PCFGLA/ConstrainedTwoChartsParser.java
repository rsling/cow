...
//  int sentencesParsed;
	...
	public ConstrainedTwoChartsParser(Grammar gr, Lexicon lex, SpanPredictor sp) {
  	...
  	scoresToAdd = new double[ArrayMath.max(numSubStatesArray)];
...
//                  if (scores[lp][rp]==null) continue;
	...
	//                    if (!allowedSubStates[start][end][pState][np]) continue;
              ...
              if (ArrayMath.max(scoresToAdd)!=0){//oScale[start][end][pState]!=Integer.MIN_VALUE && iScale[split][end][rState]!=Integer.MIN_VALUE){
              ...
              if (ArrayMath.max(unscaledScoresToAdd)!=0){//oScale[start][end][pState]!=Integer.MIN_VALUE && iScale[start][split][lState]!=Integer.MIN_VALUE){
