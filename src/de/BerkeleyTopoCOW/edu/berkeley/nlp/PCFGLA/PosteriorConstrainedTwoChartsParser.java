...
import edu.berkeley.nlp.classify.DefaultLinearizer;
...
public class PosteriorConstrainedTwoChartsParser extends
	...
	public PosteriorConstrainedTwoChartsParser(Grammar gr, Lexicon lex,
			double boost) {
		super(gr, lex, null);
		scoresToAdd1 = new double[ArrayMath.max(numSubStatesArray)];
		unscaledScoresToAdd1 = new double[scoresToAdd1.length];
		scoresToAdd2 = new double[ArrayMath.max(numSubStatesArray)];
			...
			for (int start = 0; start < (length - diff + 1); start++) {
				...
				for (int pState = 0; pState < numSubStatesArray.length; pState++) {
					...
					if (inTree) {
						...
						assert denom > 0.0;
						ArrayMath.multiplyInPlace(unscaledScoresToAdd1, 1.0 / denom);
...
//									 + " at binary phase for span " + split + " " + end);
							...
							if (ArrayMath.max(scoresToAdd1) != 0 ||ArrayMath.max(scoresToAdd2) != 0  ) {// oScale[start][end][pState]!=Integer.MIN_VALUE
								// &&
							...
							if (ArrayMath.max(unscaledScoresToAdd1) != 0 || ArrayMath.max(unscaledScoresToAdd2) != 0) {// oScale[start][end][pState]!=Integer.MIN_VALUE
								// &&
...
//							 + " at lexical phase for span " + start + " " + end);
					...
					assert denom > 0.0;
					ArrayMath.multiplyInPlace(iPosteriorScorePreU[start][end][tag], 1.0 / denom);
				}
				/*
