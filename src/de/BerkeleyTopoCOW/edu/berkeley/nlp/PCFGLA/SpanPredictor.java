...
import edu.berkeley.nlp.classify.WordInSentence;
  ...
  public double[] countGoldSpanFeatures(StateSetTreeList trainTrees){
  	...
  	if (useFirstAndLast){
  		...
  		for (int c=0; c<nWords; c++){
  			firstSum += ArrayMath.sum(firstWordCount[c]);
  			lastSum += ArrayMath.sum(lastWordCount[c]);
  		}
  	...
  	if (usePreviousAndNext){
  		...
  		for (int c=0; c<nWords; c++){
  			prevSum += ArrayMath.sum(previousWordCount[c]);
  			nextSum += ArrayMath.sum(nextWordCount[c]);
  		}
  	...
  	if (useBeginAndEndPairs){
  		...
  		for (int i=0; i<beginPairsCount.length; i++){
  			beginSum += ArrayMath.sum(beginPairsCount[i]);
  		}
  		for (int i=0; i<endPairsCount.length; i++){
  			endSum += ArrayMath.sum(endPairsCount[i]);
  		}
