...
class DependencyConverter:
    ...
    def __read_manual_heads( self, filename ):
        ...
        for line in codecs.open(filename,'r','utf-8'):
            ...
            if line.startswith('manual_head(') and line.endswith(').'):
                ids = map(int,line.partition('(')[2][:-2].split(','))
    ...
    def __convert_coordination_as_chain( self, sentence, rootid ):
        ...
        newhead = None
        for cj in conjuncts:
            if not newhead and cj.label == 'CJ' and cj.pos != 'TRUNC':
                newhead = cj
            elif not newhead:
                leftdeps.append(cj)
            else:
    ...
    def __make_empty_head( self, sentence, pcat ):
        ...
        emptynode = psnode.PSNode()
        emptynode.form = '<empty>'
        emptynode.lemma = '<empty>'
        emptynode.pos = '<EMPTY>'
        if pcat == 'S':
            emptynode.morph = [('vtype','fin')]
        elif pcat == 'VP':
            emptynode.morph = [('vtype','inf')]
        emptynode.head = -1
        emptynode.label = '_'
        emptynode.leaf = True
        emptynode.sid = sentence[0].sid
        emptynode.nid = len(sentence)+1
        return emptynode
    ...
    def __partition_list( self, pred, iterable ):
        ...
        return list(itertools.ifilter(pred,t1)), list(itertools.ifilterfalse(pred,t2))
        ...
        for node in sentence:
            if node.pos.startswith('$') and node.head == -1:
                if node.nid > 1:
                    ...
                    try:
                        node.head = itertools.dropwhile(lambda x: x.pos.startswith('$'),sentence).next().nid
    ...
    def __next_word( self, tokens ):
        try:
            return itertools.dropwhile(lambda x: x.pos.startswith('$') or not x.leaf,tokens).next()
        except StopIteration:
            return None
